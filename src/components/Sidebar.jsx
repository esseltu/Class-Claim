...
const Sidebar = ({ selectedRoom, onSelectRoom, className = '' }) => {
  ...
  return (
    <div className={`flex flex-col h-full bg-white dark:bg-gray-800 border-r border-gray-100 dark:border-gray-700 transition-colors duration-200 ${className}`}>
      ...
      <div className="px-4 pb-4 border-b border-gray-50 dark:border-gray-700">
        <div className="[&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] flex gap-2 overflow-x-auto pt-1">
          {Object.keys(ROOMS).map(block => (
            <button
              ...
              className={`
                ...
                border
                ${selectedBlock === block 
                  ? 'bg-red-600 text-white border-red-600 shadow-sm shadow-red-200/50 dark:shadow-red-900/20' 
                  : 'bg-gray-50 text-gray-600 border-gray-200 hover:bg-gray-100 dark:bg-gray-700 dark:text-gray-200 dark:border-gray-600 dark:hover:bg-gray-600'
                }
                focus:outline-none focus:ring-2 focus:ring-red-300 dark:focus:ring-red-700/40
              `}
            >
              {block}
            </button>
